...
class ParseableStringOrca {
  ...
  /**  Mapping of ORCA analyses to intermediate
   * analysis as byzantine form.  Keys are URNS for
   * an ORCA analysis record; values are URNs from the
   * Byzantine orthography collection.
   */
  LinkedHashMap byzMappings = [:]


  LinkedHashMap altMappings = [:]  


  
  
  /** Index, built from authority list, of
   * Byzantine to modern forms.
   */
  LinkedHashMap byzToModernIndex = [:]

  
  /** Index, built from authority list, of
   * alternate ortho to standard forms.
   */
  LinkedHashMap altToStandardIndex = [:]
  ...
  ParseableStringOrca(String collectionName, File byzAuthList, File altOrthList, String morphCmd) {
    ...
    byzToModernIndex = indexAuthList(byzAuthList)
    altToStandardIndex = indexAuthList(altOrthList)
  }


  /** Maps alternate forms to standard forms
   * used as the "transformed text" value for this ORCA analysis.
   * @param authList .csv file with HMT Byzantine or modern
   * orthographic equivalents. Alternate form is in second column, 
   * standard form is in third.
   * @returns A map of alternate valid forms to standard forms.
   */
  LinkedHashMap indexAuthList(File authList) {
    ...
    srcReader.readAll().each { lexLine ->
      ...
      String normalizedAlt = Normalizer.normalize(trimmed, Form.NFC)
      
      String trimmedStandard = lexLine[2].replaceAll(/^[ ]+/,"")
      trimmedStandard = trimmedStandard.replaceAll(/[ ]+$/,"")
      String normalizedStandard = Normalizer.normalize(trimmedStandard, Form.NFC)
      validForms[normalizedAlt] = normalizedStandard
    }
    return validForms
  }
   ...
   */
  void orcafyTabFile(File tabFile)
  throws Exception {
    analysisList = analyzeTabs(tabFile, 0)
  }
   ...
   */
  void orcafyTabFile(File tabFile, Integer index)
  throws Exception {
    analysisList = analyzeTabs(tabFile, index)
  }
   ...
   */
  ArrayList analyzeTabs(File tabFile, Integer index)
  throws Exception {
    ArrayList orcaList = []
	...
	orca = new Orca(orcaId, passageUrn, analysis, byzToModernIndex[normalForm] )
	byzMappings[orcaId] = lexer.byzOrthoAuthority[normalForm]
	break
